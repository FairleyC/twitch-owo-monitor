    ...
    handlebars.registerHelper('controlBar', function () {
        let output = `
          ...
          <div class="s2 middle-align right-align">
            <button class="small circle ${this.owoConnected && "tertiary"}" id ="launch-owo">
              <i>publish</i>
            </button>
            <label class="switch icon">
          ...
          <script>
            ...
            launchOwoButton.addEventListener('click', function() {
              launchOwoButton.classList.add('tertiary');
              fetch('/api/owo', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json'
              ...
              fetch('/api/health', {
                ...
                if (!response.ok) {
                  ...
                  response.json().then(function(json) {
                    queueCheckbox.checked = json.parts.queue;
                    if (launchOwoButton.classList.contains('tertiary') && json.parts.owo.vest) {
                      launchOwoButton.classList.remove('tertiary');
                    }
                    if (!json.parts.twitch.auth) {
                      const errorBar = document.getElementById('error');
                      errorBar.innerHTML = "Error: Invalid Authentication Token";
